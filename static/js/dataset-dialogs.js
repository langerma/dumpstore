...
// Dataset-scoped dialogs: delete, rename, rewrite, per-user quotas, ACL,
// chown, NFS, SMB, iSCSI. Moved out of datasets.js so both the table and the
// detail drawer can open them without an import cycle
// (datasets.js → dataset-drawer.js → dataset-dialogs.js).
...
export function openDeleteDatasetDialog(name, type) {
...
export function openRenameDatasetDialog(name) {
...
// ── Rewrite dialog (zfs rewrite — background job) ─────────────────────────────
const rewriteDialog = document.getElementById('rewriteDialog');
document.getElementById('rewriteCancelBtn').addEventListener('click', () => rewriteDialog.close());

let _rewriteDataset = '';

export function openRewriteDialog(name) {
  _rewriteDataset = name;
  document.getElementById('rewriteDatasetName').textContent = name;
  for (const id of ['rewrite-recursive', 'rewrite-skip-snap', 'rewrite-skip-clone']) {
    document.getElementById(id).checked = false;
  }
  rewriteDialog.showModal();
}

document.getElementById('rewriteStartBtn').addEventListener('click', async () => {
  const name = _rewriteDataset;
  rewriteDialog.close();
  try {
    const encodedName = name.split('/').map(encodeURIComponent).join('/');
    const data = await api('POST', '/api/rewrite/' + encodedName, {
      recursive: document.getElementById('rewrite-recursive').checked,
      skip_snapshot_shared: document.getElementById('rewrite-skip-snap').checked,
      skip_clone_shared: document.getElementById('rewrite-skip-clone').checked,
    });
    toast(`Rewrite job ${data.job_id} started on ${name} — see Jobs tab`, 'ok');
  } catch (e) {
...
export function openUserSpaceDialog(name) {
